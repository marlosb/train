...
def create_abt(QUERY_PATH, DATA_PATH, primeira_safra, ultima_safra):

    with open( QUERY_PATH, 'r' ) as open_file:
        query = open_file.read()

    primeira_safra_datetime = (
        datetime.datetime.strptime(primeira_safra, "%Y-%m-%d"))
    ultima_safra_datetime = (
        datetime.datetime.strptime(ultima_safra, "%Y-%m-%d"))
